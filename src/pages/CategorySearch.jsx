...
import { categoryFromSlug, cityFromSlug, slugify, getCategoryLocationUrl } from "../components/utils/slugUtils";
import {
...
export default function CategorySearchPage() {
  ...
  // Extraer categoría y ciudad de la URL
  const pathParts = location.pathname.split('/').filter(Boolean);
  const categorySlug = pathParts[0];
  const ciudadSlug = pathParts[1];

  const categoryName = categoryFromSlug(categorySlug);
  const ciudadName = ciudadSlug ? cityFromSlug(ciudadSlug) : null;
  ...
  const { data: profiles = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return allProfiles.filter(p => {
        ...
        const matchesCategory = !categoryName || p.categories?.includes(categoryName);
  ...
  const handleCiudadChange = (ciudad) => {
    if (ciudad === "all") {
      ...
      navigate(getCategoryLocationUrl(categoryName, ciudad));
    }
  };
  ...
  const seoDescription = ciudadName
    ? `Encuentra ${categoryName?.toLowerCase() || 'profesionales'}s verificados en ${ciudadName}. Contacta directamente, pide presupuesto gratis.`
    : `Directorio de ${categoryName?.toLowerCase() || 'profesionales'}s autónomos en España. Compara precios, lee opiniones y contacta gratis.`;

  const seoKeywords = [
    categoryName?.toLowerCase(),
    ciudadName?.toLowerCase(),
    'autónomo',
    'profesional',
    'presupuesto',
    'España'
          ...
          <Card className="mb-6 shadow-sm border-0 rounded-xl">
            <CardContent className="p-4">
              <div className="flex flex-wrap gap-3 items-center">
                ...
                <Select value={selectedCiudad} onValueChange={handleCiudadChange}>
          ...
          {!isLoading && profiles.length === 0 && (
            <Card className="p-8 text-center rounded-xl">
              ...
              <h3 className="text-xl font-bold text-gray-900 mb-2">
                No hay {categoryName?.toLowerCase() || 'profesionales'}s {ciudadName ? `en ${ciudadName}` : ''}
              </h3>
